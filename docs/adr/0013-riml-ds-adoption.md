...
2. **noter も riml-ds のトークンを使う**（**完了**、2026-09-22。ユーザー承認済み）。qrcc の段階 1 と同じく `--noter-*` を `--rd-*`（`themes/noter`）の別名にした。
   アクセントは青からティールに変わる。themes/noter を当てた状態で、本文 15.5:1（ライト）/ 18.9:1（ダーク）ほか主要な 36 通りが AAA を満たすことを計算で確かめた。
   riml-ds に対応が無いものは noter が持つ（参加者の色 8 色、エディタの地、本文の幅 70ch、ヘッダ・ツールバーの高さ）。
   エディタのダークのカーソル行は、riml-ds の danger が暗いぶん 0.3 → 0.25 に下げて構文色の 7:1 を保った（`syntax-contrast.test.ts`）。
3. **共有パッケージの部品を riml-ds の CSS クラスに寄せる**（**一部完了**、2026-09-22）。reset と noter の Avatar を寄せた。
   SkipLink・VisuallyHidden・Breadcrumbs・GlobalNav は、riml-ds の CSS に a11y の欠けがあるので据え置いた（下の表と
   [`docs/riml-ds-feedback.md`](../riml-ds-feedback.md) §5〜§8）。riml-ds 側で直れば、表に沿って置き換える。
...
| `SkipLink`（`packages/ui`）                     | `.rd-skip-link`（utilities.css）                  | **据え置き**                         | 対象サイズ 44px（AAA 2.5.5）と、地と区別する枠が無い（feedback §5）                                                                |
| `VisuallyHidden`（`packages/ui`）               | `.rd-visually-hidden`（utilities.css）            | **据え置き**                         | `:focus-within` で現れない。中にフォーカスが入ると見えなくなる（2.4.7、feedback §6）                                               |
| `ThemeToggle` と theme store（`packages/ui`）   | 対応なし                                          | —                                    | アプリの設定（保存キー、`light-dark()` の切り替え）なので残す                                                                      |
| `reset.css`（`packages/ui`）                    | `riml-ds-css/reset.css`                           | **置き換え済み**                     | reset 層の中に取り込む。class 付きリストの行頭記号を消す分だけ補う（feedback §8）                                                  |
| `Breadcrumbs`（`packages/shell`）               | `.rd-breadcrumb`（navigation.css）                | **据え置き**                         | リンクの対象サイズが 44px に届かない（ナビの中は 2.5.5 の例外にならない、feedback §7）                                             |
| `GlobalNav`（`packages/shell`）                 | `.rd-nav-menu`（navigation.css）                  | **据え置き**                         | 幅の下限が無く短い文言で 44x44 を割る。ページの帯（自前の地と罫）でヘッダ内に置けず、brand.md §4 のピルとも食い違う（feedback §7） |
...
| `Avatar`（noter）                               | `.rd-avatar`（atoms.css）                         | **置き換え済み**                     | 形は `.rd-avatar`。参加者の色・縁・大きさは noter が `data-presence` で重ねる                                                      |
...
- 段階 2 はユーザーが承認し、実施した（2026-09-22）。noter の CSS は gzip で 5.8 KB → 9.8 KB（riml-ds のトークン一式と atoms.css）。Worker のバンドルは変わらない。
