...
| URL              | 中身                                                                    |
| ---------------- | ----------------------------------------------------------------------- |
| `/`              | **生成と読み取り**。このアプリの本体。サインイン不要                    |
| `/print`         | ラベル台紙への面付け印刷                                                |
| `/nfc`           | NFC タグへの書き込み。**対応は Android の Chrome だけ**（2026-09 時点） |
| `/codes`         | 保存したコードの一覧（要サインイン）                                    |
| `/codes/$id`     | 1 件の編集と共有                                                        |
| `/shared/$token` | 共有リンクの閲覧。サインイン不要                                        |
| `/sign-in`       | サインインの選択肢                                                      |
| `/settings`      | 表示設定                                                                |
| `/api/auth/*`    | Better Auth の HTTP 入口                                                |

生成と読み取りは元は `/generate` と `/scan` に分けていたが、**どちらも
サインイン不要で端末内で完結する、このアプリの中心機能**なので、入口で
選ばせずトップに並べた。トップに来た人がそのまま両方使える形にしてある。
