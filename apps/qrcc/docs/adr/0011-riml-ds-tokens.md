...
3. riml-ds が npm に公開されるまで、`vendor/riml-ds/*.tgz` を `file:` 依存で取り込む
   ...
   スクリプトを消す。→ **2026-09-21 に完了**（riml-ds 0.2.0 が npm に公開された。`^0.2.0` を参照し、
   `vendor/` とスクリプトは削除済み。CSS の import 指定子は変わっていない）。
...
- `bun install` は npm レジストリから `@rimltempest/riml-ds-{tokens,css}` を取る（2026-09-21〜）。
  それまでは `vendor/riml-ds/*.tgz` を読んでいた——**同じファイル名で中身だけ差し替えると bun が
  キャッシュと lockfile の integrity で弾く**ので、当時は `bun update --filter @qrcc/ui <pkg>` が要った。
  npm 参照になった今はこの手間は無い
