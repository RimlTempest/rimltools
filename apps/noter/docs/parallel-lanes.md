...
- `feat/sync` は **web Worker → DO のヘッダ契約**（`docs/realtime-protocol.md` §2）を
  `features/sync/contract` に固定してから着手する。web 側（`services/web/src/server.ts`）
...
| web-shell     | `feat/web-shell`     | `features/shell/**`, `services/web/**`                         | design-system                |
| sync          | `feat/sync`          | `features/sync/**`, `services/sync/**`                         | contracts                    |
| formats       | `feat/formats`       | `features/formats/**`                                          | contracts                    |
| auth          | `feat/auth`          | `features/auth/**`, `services/web/migrations/0001_*`           | web-shell                    |
| documents     | `feat/documents`     | `features/documents/**`, `services/web/migrations/0002_*` 以降 | auth, sync                   |
...
| `services/web/src/routes.ts`                                                       | `feat/web-shell` が所有。URL 1 行の追加のみ他レーンから依頼                             |
| `services/web/wrangler.jsonc`                                                      | `feat/web-shell` が所有。binding の追加は依頼（DO binding は `feat/sync` の契約に従う） |
| `services/sync/wrangler.jsonc`                                                     | `feat/sync` が所有。**`routes` / `workers_dev` を書かない**                             |
| `services/web/migrations/`                                                         | 連番はレーン順（auth = 0001、documents = 0002〜）。番号衝突は rebase 時に後発が振り直す |
...
cd <リポジトリ直下>/.claude/worktrees/noter/feat-editor/apps/noter
...
| `markuplint`              | `**/*.tsx`                                                                                                             |
| `a11y` (Playwright + axe) | `services/web/**`, `shared/ui/**`, `features/*/ui/**`                                                                  |
| `guard`                   | 常に（`services/sync` に `routes` / `workers_dev` が無い、`new_classes` を使っていない、`class` が許可場所以外に無い） |
