...
- [ ] I/O（fetch, D1, Durable Object storage, WebSocket, Date, crypto, random）を引数で受け取っている
...
- [ ] 1 ファイルの責務が 1 つ（SRP）
- [ ] 新しい種類（qrcc: symbology / payload、noter: 文書種別）を「新ファイル + レジストリ 1 行」で足せる（OCP）
...
- [ ] テストサイズが適切（`.claude/skills/rimltools-tdd` 参照）
- [ ] 境界値を含む（空・最大長ちょうど / +1 など。プロダクト別の具体例は `<tool>-conventions`）
