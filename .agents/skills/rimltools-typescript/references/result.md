...
`Result` を境界の表現（HTTP レスポンス、WebSocket の close code、Rust の `Result`）に
変換する場所は 1 か所に決める。具体的な対応表はプロダクトごと:

- qrcc: Rust engine との JSON 対応 → `qrcc-conventions` の `references/typescript.md`
- noter: Durable Object / WebSocket の close code → `noter-conventions` の `references/typescript.md`
