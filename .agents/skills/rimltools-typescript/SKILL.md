---
name: rimltools-typescript
description: RimlTools（qrcc・noter ほか全プロダクト共通）の TypeScript コーディング規約。TS/TSX を書く・直す・レビューする前に必ず読む。any/as/class/enum/! を禁止し、Result・Branded 型・discriminated union・Utility/Conditional Types でユースケースを型に落とし込み、依存は関数引数で注入する。「型が決まらない」「as で通した」「クラスにするか迷う」「エラーをどう返すか」「モックできない」ときに発火。
---
...
ここに書かれた禁止事項は `.oxlintrc.json` の `rimltools/*` ルール（`tools/oxlint-plugin-rimltools`）で機械的に強制される。
...
唯一の例外は Cloudflare の要件で class が必須な Durable Object の殻（noter。`noter-conventions` 参照）。
...
```ts
// 例（qrcc）
const ErrorCorrection = { L: 'L', M: 'M', Q: 'Q', H: 'H' } as const
type ErrorCorrection = (typeof ErrorCorrection)[keyof typeof ErrorCorrection]
```
...
| 道具                           | 使いどころ                                                      |
| ------------------------------ | --------------------------------------------------------------- |
| Branded type (`unique symbol`) | ID・検証済み文字列。`CodeId` と `UserId` を取り違えられなくする |
| Discriminated union            | 「状態」「種類」。symbology・payload・接続状態・UI state        |
| Utility Types                  | 既存型からの派生（`Omit`/`Pick`/`Readonly`/`Extract`）          |
| Conditional / Mapped Types     | 種類ごとに異なるオプションを 1 つの型関数で表す                 |

**原則: 不正な状態を表現できない型にする。**
例（qrcc）: 「`kind: 'qr'` のときだけ `errorCorrection` がある」なら、フラットな
optional ではなく union のメンバーとして持つ。

## 3. エラーは値。`Result<T, E>`

ドメイン層（`shared/contract`, `features/<name>/contract`, `features/<name>/core`, `*/engine`, `packages/*/src/core`）では `throw` 禁止
（`rimltools/no-throw-in-domain`）。失敗は戻り値で表す。
...
type Deps = {
  readonly now: () => Date
  readonly saveCode: (c: Code) => Promise<Result<CodeId, SaveError>>
}
export const createCode =
  (deps: Deps) =>
  async (input: NewCode): Promise<Result<CodeId, CreateError>> => {
    /* ... */
  }
```
...
- 配線は composition root だけ（qrcc: `apps/*/src/server/container.ts`、noter: `features/<name>/ui/*-wiring.route.ts` と `apps/sync/src/index.ts`）。
...
- **OCP**: 種類の追加（qrcc の symbology、noter の文書種別など）が「新ファイル + レジストリに 1 行」で済むこと。
...
共通の層（詳細な一覧はプロダクトの `<tool>-architecture` / `<tool>-conventions`）:
...
features/<name>/server    … D1 / Better Auth などのサーバ側アダプタ
features/<name>/ui        … UI。ドメイン型をそのまま使う
apps/*/src/server         … I/O（D1・service binding・DO binding）と composition root
packages/*                … プロダクト横断の共有パッケージ（`@rimltools/*`）
```

- `import/no-cycle` は error。
- feature 同士・プロダクトと packages の間は、`@<tool>/<name>/<subpath>` / `@rimltools/<name>` の
  **公開サブパス経由でのみ**依存する。相対パスで隣の feature や他プロダクトに手を伸ばさない。
