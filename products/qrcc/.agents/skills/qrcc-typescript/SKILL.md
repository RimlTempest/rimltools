---
name: qrcc-typescript
description: qrcc2 の TypeScript コーディング規約。TS/TSX を書く・直す・レビューする前に必ず読む。any/as/class/enum/! を禁止し、Result・Branded 型・discriminated union・Utility/Conditional Types でユースケースを型に落とし込み、依存は関数引数で注入する。「型が決まらない」「as で通した」「クラスにするか迷う」「エラーをどう返すか」「モックできない」ときに発火。
---

# qrcc2 TypeScript 規約

型でユースケースを表現し、実行時に「起こりえない状態」をコンパイル時に消す。
ここに書かれた禁止事項は `.oxlintrc.json` の `qrcc/*` ルールで機械的に強制される。
...
- 配線は composition root（`apps/*/src/server/container.ts` など）1 箇所だけ。
...
- **OCP**: 新 symbology の追加が「新ファイル + レジストリに 1 行」で済むこと。
