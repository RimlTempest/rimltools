 ...
 * 引き込まずに済み、テストでは素のオブジェクトで同じ形を満たせる。
...
export type SqlRunner = {
  ...
  /** 変更行数を返す単発の実行。制約違反は 0 行ではなく失敗になる。 */
  readonly run: (statement: SqlStatement) => Promise<Result<number, SqlError>>
  /**
...
export const makeD1SqlRunner = (db: D1Like): SqlRunner => {
  ...
  const run = async (statement: SqlStatement): Promise<Result<number, SqlError>> => {
    try {
      const outcome = await prepared(statement).all()
      return ok(outcome.meta?.changes ?? 0)
    } catch (cause) {
      return err({ kind: 'storage_unavailable', detail: String(cause) })
    }
  }
