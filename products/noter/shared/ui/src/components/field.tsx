...
export const Field = (props: FieldProps) => {
  ...
  const shared = {
    id: controlId,
    className: 'noter-field__control',
    ...(describedBy === '' ? {} : { 'aria-describedby': describedBy }),
    ...(error === undefined ? {} : { 'aria-invalid': true }),
  }

  const wrap = (control: ReactNode) => (
    <div className="noter-field">
      <label className="noter-field__label" htmlFor={controlId}>
        {label}
      </label>
      {hint === undefined ? undefined : (
        <span className="noter-field__hint" id={hintId}>
          {hint}
        </span>
      )}
      {control}
      {error === undefined ? undefined : (
        <span className="noter-field__error" id={errorId}>
          {error}
        </span>
      )}
    </div>
