---
name: noter-worktree
description: noter の worktree 並行作業ルール。複数の作業を同時に進めるとき、新しいレーンを始めるとき、rebase が競合したとき、PR を出すときに読む。レーンごとの所有ディレクトリ・依存順・共有ファイルの再生成規約を定義する。「並行で進めたい」「worktree を作る」「コンフリクトした」「どのブランチで作業する」で発火。
---
...
bun run wt new feat/editor   # worktree 作成（依存インストール・フック設定込み）
...
bun run wt done feat/editor  # マージ後に片付け
```
...
| `.oxlintrc.json` / `lefthook.yml` / `tsconfig.json` | `chore/devops` のみ                                               |
...
- `docs/architecture.md` と `docs/domain-model.md`（同期に触るなら `docs/realtime-protocol.md` も）
- 該当スキル（`noter-typescript` / `noter-html-a11y` / `noter-tdd` / `noter-architecture`）
