...
| ツール | できること                                               | URL                          | ソース                               |
| ------ | -------------------------------------------------------- | ---------------------------- | ------------------------------------ |
| qrcc   | QR・バーコードの生成 / 読み取り / 管理 / ラベル印刷      | <https://qrcc.riml4i.com>    | [`products/qrcc`](products/qrcc)     |
| noter  | リアルタイム共同編集のエディタ（Markdown・Mermaid ほか） | <https://noter.riml4i.com>   | [`products/noter`](products/noter)   |
| portal | ツールの一覧ページ                                       | `tools.riml4i.com`（公開前） | [`products/portal`](products/portal) |
...
| qrcc                                                                                                               | noter                                                                                                        | portal                                                                                                                                            |
| ------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| [![qrcc の構成図](products/qrcc/docs/architecture/qrcc2-architecture.light.png)](products/qrcc/README.md#技術構成) | [![noter の構成図](products/noter/docs/architecture/noter-architecture.light.png)](products/noter/README.md) | [![portal の構成図](products/portal/docs/architecture/portal-architecture.light.png)](products/portal/docs/architecture/portal-architecture.html) |
...
- **無料枠で運用しきる。** Workers Free・D1・Grafana Cloud Free・public の GitHub だけを使い、R2 と KV は使わない
  （[qrcc ADR-0009](products/qrcc/docs/adr/0009-stay-on-workers-free.md)）
- **計算はブラウザで。** QR の生成・読み取りは Rust を wasm にして端末で動かし、Worker を消費しない
  （[qrcc ADR-0003](products/qrcc/docs/adr/0003-rust-core-dual-target.md)）
...
```
products/<tool>/   ツール本体（qrcc・noter・portal）。詳細は各ツールの README
packages/          ツール間で共有するパッケージ（contract・ui・shell・auth・telemetry・flags）
scripts/           リリース・監視・CI・構成図のスクリプト（TypeScript）
infra/             OpenTofu（terraform・grafana）、state の置き場（tfstate）、暗号化した秘密（secrets）
observability/     ダッシュボードの正本と、ローカルの LGTM（docker compose）
flags/             feature flag の定義（ツールごとの JSON）
docs/              全体の設計・ADR・runbook
...
```bash
cp products/qrcc/apps/web/.dev.vars.example products/qrcc/apps/web/.dev.vars
cp products/noter/.dev.vars.example products/noter/apps/web/.dev.vars

bun run --cwd products/qrcc/apps/web db:local
bun run --cwd products/noter/apps/web db:local

bun run --cwd products/qrcc/apps/api build   # qrcc だけ。Rust の api Worker を 1 回ビルドしておく
...
```bash
docker compose -f observability/local/compose.yaml up -d   # docker compose が無ければ docker-compose
bun scripts/observability/local-smoke.ts   # 受け取れているかを確かめる
open http://127.0.0.1:3000
```

`.dev.vars` に書く送り先は [`observability/local/README.md`](observability/local/README.md) にある。
...
bun run --cwd products/qrcc a11y   # Playwright + axe（WCAG AAA タグ込み）
bun run --cwd products/noter e2e
```
...
- 並行作業は worktree のレーンで行う: `bun run --cwd products/<tool> wt list`
...
| [`docs/observability.md`](docs/observability.md)                 | 計装（OTLP・Faro）とサンプリング                                |
| [`docs/observability-grafana.md`](docs/observability-grafana.md) | Grafana の構成・アラート・ローカルでの確認                      |
...
| [`docs/adr/`](docs/adr/)                                         | 横断の設計判断（各ツールの ADR は `products/<tool>/docs/adr/`） |
