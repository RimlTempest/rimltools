...
**どのリクエストをどう扱うか（方針）は各プロダクトの `apps/web/src/sw/policy.ts` が決める。**
...
各プロダクトは `apps/web/src/sw/sw.ts` から `startServiceWorker` を呼ぶ。`bun run sw`
（`dev` / `build` の前に自動で走る）が `scripts/build-sw.ts` で 1 ファイルの JS（IIFE）にして
`apps/web/public/sw.js` に書き出す。**`public/sw.js` は生成物なのでコミットしない。**

- 新しい版をその場で有効化（skipWaiting）はしない。開いているページの途中でアセットの世代が入れ替わると壊れる
- activate で、自分の版（`cacheName`）以外のキャッシュを消す。方針を変えたら `cacheName` を上げる
- `sw.ts` は webworker の型で検査する（`apps/web/tsconfig.sw.json`）。アプリ本体の tsconfig（DOM の型）には入れない
