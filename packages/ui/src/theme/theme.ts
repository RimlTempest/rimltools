...
const makeThemeStoreFor =
  (storageKey: string) =>
  (storage: StorageLike, root: ThemeRoot): ThemeStore => {
...
/**
 * プロダクトごとの保存キー（例: `qrcc-theme`）で、ストアと初期化スクリプトを作る。
 * 保存キーを変えると既存の利用者の選択が消えるので、プロダクトは今のキーを渡し続ける。
 */
export const createThemeKit = (storageKey: string) => ({
  THEME_STORAGE_KEY: storageKey,
  makeThemeStore: makeThemeStoreFor(storageKey),
  themeInitScript: themeInitScriptFor(storageKey),
})
