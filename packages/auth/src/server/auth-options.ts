 ...
 *
 * better-auth は**型としてだけ**参照する。実行時の部品（anonymous プラグイン）は
 * プロダクトが注入する。各プロダクトの better-auth の版（package.json で固定）を
...
export type AnonymousPlugin = typeof anonymous
...
export type AuthProductSettings = {
  ...
  readonly anonymous: AnonymousPlugin
}
