 ...
 *
 * better-auth には**依存しない**（型も含めて）。実行時の部品（anonymous プラグイン）と
 * データベースのアダプタはプロダクトが注入し、戻り値が `BetterAuthOptions` を満たすかは
 * プロダクト側で `satisfies` する。各プロダクトの better-auth の版（package.json で固定）を
 * そのまま使い、1 つの Worker に 2 つの版が混ざらないようにするため。
 */
...
/** anonymous プラグインに渡す設定のうち、ここで決めるものだけ。 */
export type AnonymousSettings = {
  readonly emailDomainName?: string
  readonly generateName: () => string
  readonly onLinkAccount: (linked: LinkedAccounts) => Promise<void>
}

/** better-auth の `anonymous` と同じ形の関数。プロダクトが自分の版のものを渡す。 */
export type AnonymousPlugin<Plugin> = (settings: AnonymousSettings) => Plugin
...
export type AuthOptionsDeps<Database> = {
...
export type AuthProductSettings<Plugin> = {
  readonly appName: string
  /** Better Auth の既定と同じ値でも、明示したいプロダクトだけが書く。 */
  readonly basePath?: string
  /** ゲストに割り当てるメールアドレスのドメイン。実在しない TLD（`.invalid`）にする。 */
  readonly guestEmailDomain?: string
  readonly anonymous: AnonymousPlugin<Plugin>
}

export type SharedAuthOptionsDeps<Database, Plugin> = AuthOptionsDeps<Database>
  & AuthProductSettings<Plugin>
...
export const buildAuthOptions = <Database, Plugin>(
  deps: SharedAuthOptionsDeps<Database, Plugin>,
) => ({
  ...
  emailAndPassword: { enabled: false as const },
  socialProviders:
