 ...
 *   DELETE /states/<name>?ID=<id>  state を消す（ロックの持ち主だけ）
...
const METHODS = new Set(['GET', 'POST', 'DELETE', 'LOCK', 'UNLOCK'])
...
const route = async (request: Request, deps: HandlerDeps): Promise<Response> => {
  ...
  if (!role.ok) {
    if (role.error === 'misconfigured') return text(500, 'backend credentials are not configured')
    if (role.error === 'forbidden') return text(403, 'read-only credentials')
    return text(401, 'unauthorized', { 'www-authenticate': 'Basic realm="rimltools-tfstate"' })
  }
  ...
  // POST / DELETE: ロックの持ち主（?ID= が一致）だけが書ける。
  ...
  if (method === 'DELETE') {
    const deleted = await store.deleteState(path.value)
    return deleted.ok ? text(200, 'deleted') : text(500, 'storage error')
  }
...
export const handle = async (request: Request, deps: HandlerDeps): Promise<Response> => {
  ...
  // 本文と資格情報は絶対に出さない。path・method・結果・呼び出し元だけ
  deps.log({
    event: 'tfstate',
